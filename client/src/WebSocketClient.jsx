...
function ProfileModal({ onClose, authToken, user, onUpdate }) {
  ...
  const save = async () => { setSaving(true); try { const res = await fetch(`/auth/profile`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: `Bearer ${authToken}` }, body: JSON.stringify({ displayName, bio }) }); const data = await res.json(); onUpdate(data); } catch { } setSaving(false); onClose(); };
  const uploadAvatar = async (file) => { const fd = new FormData(); fd.append("file", file); try { const res = await fetch(`/auth/profile/avatar`, { method: "POST", headers: { Authorization: `Bearer ${authToken}` }, body: fd }); const data = await res.json(); onUpdate({ avatarUrl: data.avatarUrl }); } catch { } };
  return (
        ...
        <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
          <div style={{ position: "relative", cursor: "pointer" }} onClick={() => fileRef.current.click()}>
            <div style={{ width: 72, height: 72, borderRadius: "50%", background: "var(--bubble-me)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, fontWeight: 700, color: "#fff", overflow: "hidden" }}>
              {user?.avatarUrl ? <img src={user.avatarUrl.startsWith("http") ? user.avatarUrl : `${user.avatarUrl}`} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => e.target.style.display = "none"} /> : (user?.username || "?")[0].toUpperCase()}
            </div>
...
export default function WebSocketClient({ authUser, authToken, onLogout }) {
  ...
  useEffect(() => {
    if (authUser) {
      ...
      authedFetch(`/auth/users`).then(setAllUsers).catch(() => {});
      authedFetch(`/auth/profile/${authUser}`).then(setMyProfile).catch(() => {});
      authedFetch(`/rooms`).then(setRooms).catch(() => {});
    }
  }, []); // eslint-disable-line
  ...
  const connect = () => {
    ...
    const client = new Client({
      ...
      onConnect: async () => {
        try {
          const res = await fetch("/auth/history", { headers: { Authorization: `Bearer ${authToken}` } });
  ...
  const renderContent = (msg) => {
    ...
    const resolveUrl = (url) => { if (!url) return ""; if (url.startsWith("http")) { const path = url.replace(/^https?:\/\/[^/]+/, ""); return `${path}`; } return `${url.startsWith("/") ? "" : "/"}${url}`; };
          ...
          <div className="sidebar" style={{ flexDirection: "column", background: "var(--glass2)", borderRight: "1px solid var(--divider)", width: 240, minWidth: 240, maxWidth: 240, flexShrink: 0, overflow: "hidden", display: (window.innerWidth < 640 && view !== "channel" && view !== "friends") ? "none" : "flex" }}>
            ...
            <div style={{ padding: "8px 10px", borderTop: "1px solid var(--divider)", display: "flex", alignItems: "center", gap: 8, flexShrink: 0, background: "var(--glass2)", position: "relative" }}>
                  ...
                  <div style={{ height: 56, background: "linear-gradient(135deg,#5865f2,#7b8cff)", position: "relative", flexShrink: 0 }}>
                    <div style={{ position: "absolute", bottom: -22, left: 12, width: 50, height: 50, borderRadius: "50%", background: dark ? "#18141f" : "#f2f0ff", padding: 3, boxSizing: "border-box" }}>
                      <div style={{ width: "100%", height: "100%", borderRadius: "50%", background: "var(--bubble-me)", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 700, color: "#fff", position: "relative" }}>
                        {myProfile?.avatarUrl ? <img src={myProfile.avatarUrl.startsWith("http") ? myProfile.avatarUrl : `${myProfile.avatarUrl}`} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : name[0]?.toUpperCase()}
              ...
              <div onClick={e => { e.stopPropagation(); setShowProfilePopup(v => !v); setShowStatusMenu(false); }} style={{ position: "relative", width: 34, height: 34, borderRadius: "50%", background: "var(--bubble-me)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff", cursor: "pointer", flexShrink: 0, overflow: "hidden" }}>
                {myProfile?.avatarUrl ? <img src={myProfile.avatarUrl.startsWith("http") ? myProfile.avatarUrl : `${myProfile.avatarUrl}`} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => { e.target.style.display = "none"; }} /> : name[0]?.toUpperCase()}
